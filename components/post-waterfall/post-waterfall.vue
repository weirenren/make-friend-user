				...
				<view @click="jump(item)" class="demo-warter warter-left" v-if="item.media&&item.media.length > 0" v-for="(item, index) in leftList" :key="index">
					...
					<block v-else>
						<view class="video-wrap">
							...
							<!-- 如果使用七牛云存储打开如下两行代码注释 -->
					        <image class="cover-img" mode="aspectFill" v-if="item.cut == 1" :src="item.media[0]"></image>
							<image class="cover-img" mode="aspectFill" v-else :src="item.media[0] + '?vframe/jpg/offset/1'"></image>
