				...
				<view @click="onClick" class="field-body">
					...
					<block v-else>
						<input
							...
							:placeholder="placeholder"
						/>
						<view v-if="type == 'select'" class="right" @tap="onRightIcon"><q-icon name="q-sorting"></q-icon></view>
						<view v-if="rightIcon" class="right" @tap="onRightIcon"><q-icon :name="rightIcon"></q-icon></view>
					</block>
				</view>
			</view>
		</view>
		<!-- Select选择弹窗 -->
		<q-select v-model="showPopup" :mode="mode" @confirm="onSelectConfirm" :list="list"></q-select>
	</view>
</template>
