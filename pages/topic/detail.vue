	...
	<view class="wrap">
		...
		<view class="post-top-box">
			<navigator
				...
				v-for="(item, index) in topicInfo.topPost"
				:key="index"
			>
		...
		<view class="member-wrap" v-if="topicInfo.discussList.length > 0">
			...
			<scroll-view :scroll-x="true" style="height: 180rpx;">
				<view class="dis-wrap">
					<navigator :url="'/pages/discuss/detail?id=' + item.id" class="d-item" v-for="(item, index) in topicInfo.discussList" :key="item.id">
		...
		<view v-show="current == 0">
			<post-list :showTopic="false" :handle="true" :uid="topicInfo.uid" :list="postNews" :loadStatus="loadStatus" :admin="topicInfo.isAdmin"></post-list>
		</view>
		<view v-show="current == 1">
			<post-list :showTopic="false" :handle="true" :uid="topicInfo.uid" :list="postHot" :loadStatus="loadStatus" :admin="topicInfo.isAdmin"></post-list>
		</view>
		<!-- 底部菜单 -->
		<view class="tabbar">
		...
		<!-- 加入圈子弹窗 -->
		<u-modal v-model="joinTopicModel" :show-cancel-button="true" confirm-text="加入圈子" :content="'是否加入【' + topicInfo.topicName + '】?'" @confirm="joinTopic"></u-modal>
		<!-- 解散圈子弹窗 -->
		<u-modal
			...
			confirm-color="red"
			confirm-text="确认"
			:content="'解散【' + topicInfo.topicName + '】后，将不可恢复，是否确认解散？'"
			@confirm="topicDel"
		></u-modal>
		<!-- 选择分享弹窗 -->
...
export default {
	...
	methods: {
		...
		async shareCanvas() {
			...
			let txtWith = context.measureText(this.topicInfo['topicName']).width;
			context.setFontSize(20);
			context.fillText(this.topicInfo['topicName'], txtWith / 2, 40, 280);
		...
		onTrigger(type) {
			...
			if (type == 3) {
				...
				uni.navigateTo({
					url: '/pages/post/add?topicId=' + this.topicId + '&topicName=' + this.topicInfo.topicName + '&type=' + type
				});
			}
		},
		jump(uid) {
