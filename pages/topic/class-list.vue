					...
					<block v-for="(item, index) in topicList" :key="index">
						<navigator :url="'/pages/topic/detail?id=' + item.id">
							<view class="topic-item">
								<u-image width="100rpx" height="100rpx" shape="circle" :src="item.coverImage"></u-image>
								<view class="right">
									<view>{{ item.topicName }}</view>
									<view class="desc">{{ item.description.substring(0, 10) }}</view>
									<view class="num">
										<text>{{ item.userCount }}人已加入</text>
			...
			this.$H.get('topic/classList').then(res => {
				...
				if (this.classId) {
					this.classList.forEach((item, index) => {
						if (item.cateId == that.classId) {
							this.classId = res.result[index].cateId;
							this.swichMenu(index);
						}
					});
				} else {
					this.classId = res.result[0].cateId;
					this.getTopicList();
				}
			});
		},
